...
namespace Microsoft.Dafny.Core {
  public class Printer {
    ...
    public void PrintTopLevelDecls(List<TopLevelDecl> decls, int indent, List<string>/*?*/ prefixIds, string fileBeingPrinted) {
      ...
      foreach (TopLevelDecl d in decls) {
        ...
        if (d is OpaqueTypeDecl) {
          ...
          if (d is LiteralModuleDecl) {
            ...
            wr.Write(" {0} ", dd.Name);
            wr.Write("= {0}", Util.Comma(".", dd.Path, id => id));
            if (dd.Exports.Count > 0) {
              wr.Write("`{{{0}}}", Util.Comma(",", dd.Exports, id => id));
            }
            wr.WriteLine();
    ...
    public void PrintModuleDefinition(ModuleDefinition module, VisibilityScope scope, int indent, List<string>/*?*/ prefixIds, string fileBeingPrinted) {
      Contract.Requires(module != null);
      Contract.Requires(0 <= indent);
    ...
    bool ParensMayMatter(Expression expr) {
      Contract.Requires(expr != null);
      return expr is MemberSelectExpr mse && mse.Member is Function;
    }
