          ...
          for (int s = 0; s < stmtList.Count; s++) {
            ...
            if (updStmt != null && updStmt.IsInoutAssign) {
              ...
              if (exprRhs == null) {
                ...
                // TODO(andrea)
                if (updStmt.InoutAssign == InoutAssign.Ghost) {
                  reporter.Warning(MessageSource.Rewriter, m.tok,
                    "possibly unsound: inout ghost does not yet check that the target is ghost");
                }
