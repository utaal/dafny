...
namespace Microsoft.Dafny {
  public abstract class Compiler {
    ...
    /// "tok" is allowed to be null if "type" is.
    /// </summary>
    protected virtual void DeclareLocalVar(string name, Type /*?*/ type, Bpl.IToken /*?*/ tok, Expression rhs, TargetWriter wr) {
    ...
    protected bool UseReturnStyleOuts(Method m, int nonGhostOutCount) {
      if (nonGhostOutCount <= 1) {
        return true;
      }
      switch (TargetLanguage.MultipleReturnStyle) {
        case TargetLanguage.MultipleReturnStyleEnum.MultipleReturns:
        case TargetLanguage.MultipleReturnStyleEnum.Tuple:
          return true;
        case TargetLanguage.MultipleReturnStyleEnum.OutParameters:
          return false;
        default:
    ...
    protected bool SupportsMultipleReturns {
      get {
        return TargetLanguage.MultipleReturnStyle == TargetLanguage.MultipleReturnStyleEnum.MultipleReturns;
      }
    }
    ...
    /// nothing.
    protected virtual TargetWriter EmitMethodCallModifiers(Method method, List<Type> lhsTypes, TargetWriter wr) {
      return wr;
    }
    ...
    protected void EmitAssignmentRhs(Expression rhs, TargetWriter wr) {
      var w = EmitAssignmentRhs(wr);
      TrExpr(rhs, w);
    }
    ...
    protected virtual void EmitReturnExpr(Expression expr, TargetWriter wr) {  // emits "return <expr>;" for function bodies
    ...
    protected delegate void FCE_Arg_Translator(Expression e, TargetWriter wr);

    protected abstract void EmitRotate(Expression e0, Expression e1, bool isRotateLeft, TargetWriter wr, FCE_Arg_Translator tr);
    /// <summary>
    ...
    protected virtual void EmitITE(Expression guard, Expression thn, Expression els, TargetWriter wr) {
    ...
    protected abstract TargetWriter EmitArraySelect(List<Expression> indices, Type elmtType, TargetWriter wr);
    ...
    protected abstract void EmitExprAsInt(Expression expr, TargetWriter wr);
    protected abstract void EmitIndexCollectionSelect(Expression source, Expression index, TargetWriter wr);
    protected abstract void EmitIndexCollectionUpdate(Expression source, Expression index, Expression value, TargetWriter wr, bool nativeIndex = false);
    ...
    /// </summary>
    protected abstract void EmitSeqSelectRange(Expression source, Expression/*?*/ lo, Expression/*?*/ hi, bool fromArray, TargetWriter wr);
    protected abstract void EmitSeqConstructionExpr(SeqConstructionExpr expr, TargetWriter wr);
    protected abstract void EmitMultiSetFormingExpr(MultiSetFormingExpr expr, TargetWriter wr);
    protected abstract void EmitApplyExpr(Type functionType, Bpl.IToken tok, Expression function, List<Expression> arguments, TargetWriter wr);
    protected abstract TargetWriter EmitBetaRedex(List<string> boundVars, List<Expression> arguments, string typeArgs, List<Type> boundTypes, Type resultType, Bpl.IToken resultTok, TargetWriter wr);
    ...
    protected abstract TargetWriter CreateIIFE_ExprBody(Expression source, Type sourceType, Bpl.IToken sourceTok, Type resultType, Bpl.IToken resultTok, string bvName, TargetWriter wr);  // Immediately Invoked Function Expression
    ...
    protected abstract void EmitUnaryExpr(ResolvedUnaryOp op, Expression expr, TargetWriter wr);
    ...
    protected abstract void EmitConversionExpr(ConversionExpr e, TargetWriter wr);
    protected abstract void EmitCollectionDisplay(CollectionType ct, Bpl.IToken tok, List<Expression> elements, TargetWriter wr);  // used for sets, multisets, and sequences
    protected abstract void EmitMapDisplay(MapType mt, Bpl.IToken tok, List<ExpressionPair> elements, TargetWriter wr);
    protected abstract void EmitCollectionBuilder_New(CollectionType ct, Bpl.IToken tok, TargetWriter wr);
    protected abstract void EmitCollectionBuilder_Add(CollectionType ct, string collName, Expression elmt, TargetWriter wr);
    protected abstract TargetWriter EmitMapBuilder_Add(MapType mt, Bpl.IToken tok, string collName, Expression term, TargetWriter wr);
    ...
    protected abstract void EmitSingleValueGenerator(Expression e, string type, TargetWriter wr);
    ...
    public void Compile(Program program, TargetWriter wrx) {
      ...
      PreCompiler.PreCompile(program, TargetLanguage, idGenerator);
    ...
    void TrCasePatternOpt<VT>(CasePattern<VT> pat, Expression rhs, TargetWriter wr) where VT: IVariable {
      TrCasePatternOpt(pat, rhs, null, rhs.Type, rhs.tok, wr);
    }

    void TrCasePatternOpt<VT>(CasePattern<VT> pat, Expression rhs, string rhs_string, Type rhsType, Bpl.IToken rhsTok, TargetWriter wr) where VT: IVariable {
      ...
      if (pat.Var != null) {
        ...
        if (rhs != null) {
          DeclareLocalVar(tmp_name, rhs.Type, rhs.tok, rhs, wr);
        } else {
        ...
        for (int i = 0; i < pat.Arguments.Count; i++) {
          ...
          if (formal.IsGhost) {
            ...
            TrCasePatternOpt(arg, null, sw.ToString(), pat.Expr.Type, pat.Expr.tok, wr);
            k++;
          }
        }
      }
    ...
    void TrExprOpt(Expression expr, TargetWriter wr) {
      Contract.Requires(expr != null);
      if (expr is LetExpr) {
        var e = (LetExpr)expr;
        if (e.Exact) {
          for (int i = 0; i < e.LHSs.Count; i++) {
            ...
            if (Contract.Exists(lhs.Vars, bv => !bv.IsGhost)) {
              TrCasePatternOpt(lhs, e.RHSs[i], wr);
            }
          }
          TrExprOpt(e.Body, wr);
        } else {
        ...
        DeclareLocalVar(source, e.Source.Type, e.Source.tok, e.Source, wr);
    ...
    void TrStmt(Statement stmt, TargetWriter wr) {
      ...
      if (stmt is PrintStmt) {
        ...
        Contract.Assert(resolved.Count == 1); // the pre-compiler should have taken care of this
        ...
        if (s.AssumeToken != null) {
          ...
          if (missingBounds.Count != 0) {
            ...
            TrAssignSuchThat(lhss, s.Expr, s.Bounds, s.Tok.line, wr);
          }
        }
        ...
        if (s.Guard == null) {
          ...
          TrExpr(s.IsBindingGuard ? Translator.AlphaRename((ExistsExpr)s.Guard, "eg_d") : s.Guard, guardWriter);
        ...
        foreach (var alternative in s.Alternatives) {
          TargetWriter guardWriter;
          var thn = EmitIf(out guardWriter, true, wr);
          TrExpr(alternative.IsBindingGuard ? Translator.AlphaRename((ExistsExpr)alternative.Guard, "eg_d") : alternative.Guard, guardWriter);
        ...
        if (s.Alternatives.Count != 0) {
          ...
          foreach (var alternative in s.Alternatives) {
            ...
            TrExpr(alternative.Guard, guardWriter);
            TrStmtList(alternative.Body, thn);
          }
        ...
        if (s.Cases.Count != 0) {
          string source = idGenerator.FreshId("_source");
          DeclareLocalVar(source, s.Source.Type, s.Source.tok, s.Source, wr);
        ...
        if (Contract.Exists(s.LHS.Vars, bv => !bv.IsGhost)) {
          TrCasePatternOpt(s.LHS, s.RHS, wr);
        }
    ...
    protected virtual TargetWriter EmitIngredients(TargetWriter wr, string ingredients, int L, string tupleTypeArgs, ForallStmt s, AssignStmt s0, Expression rhs){
      ...
      using (var wrTuple = EmitAddTupleToList(ingredients, tupleTypeArgs, wr)){
        if (s0.Lhs is MemberSelectExpr){
          var lhs = (MemberSelectExpr) s0.Lhs;
          TrExpr(lhs.Obj, wrTuple);
        }
        else if (s0.Lhs is SeqSelectExpr){
          var lhs = (SeqSelectExpr) s0.Lhs;
          TrExpr(lhs.Seq, wrTuple);
          wrTuple.Write(", ");
          EmitExprAsInt(lhs.E0, wrTuple);
        }
        else{
          var lhs = (MultiSelectExpr) s0.Lhs;
          ...
          for (int i = 0; i < lhs.Indices.Count; i++){
            wrTuple.Write(", ");
            EmitExprAsInt(lhs.Indices[i], wrTuple);
          }
        }
    ...
    protected TargetWriter CompileGuardedLoops(List<BoundVar> bvs, List<ComprehensionExpr.BoundedPool> bounds, Expression range, TargetWriter wr) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        CompileCollection(bound, bv, false, collectionWriter, bounds, bvs, i);
      }
      ...
      foreach (var bv in bvs) {
        ...
        TrParenExpr(bvConstraints, guardWriter);
        guardWriter.Write(" && ");
      }
      TrParenExpr(range, guardWriter);

      return wr;
    }

    void CompileCollection(ComprehensionExpr.BoundedPool bound, IVariable bv, bool includeDuplicates,
        TargetWriter collectionWriter,
      ...
      if (bound is ComprehensionExpr.BoolBoundedPool) {
        ...
        EmitSingleValueGenerator(b.E, TypeName(b.E.Type, collectionWriter, b.E.tok), collectionWriter);
      } else if (bound is ComprehensionExpr.SetBoundedPool) {
        var b = (ComprehensionExpr.SetBoundedPool)bound;
        TrParenExpr(b.Set, collectionWriter);
        ...
        TrParenExpr(b.MultiSet, collectionWriter);
    ...
    private void IntroduceAndAssignBoundVars(ExistsExpr exists, TargetWriter wr) {
      ...
      TrAssignSuchThat(ivars, exists.Term, exists.Bounds, exists.tok.line, wr);
    }
    ...
    private void TrAssignSuchThat(List<IVariable> lhss, Expression constraint, List<ComprehensionExpr.BoundedPool> bounds, int debuginfoLine, TargetWriter wr) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        CompileCollection(bound, bv, true, collectionWriter);
        if (needIterLimit) {
    ...
    private string StabilizeExpr(Expression e, string prefix, TargetWriter wr) {
      if (IsStableExpr(e)) {
        ...
        DeclareLocalVar(v, null, null, e, wr);
        return v;
      }
    }
    ...
    void TrCallStmt(CallStmt s, string receiverReplacement, TargetWriter wr) {
      ...
      if (s.Method == enclosingMethod && enclosingMethod.IsTailRecursive) {
        ...
        if (receiverReplacement != null) {
          ...
          inTmps.Add(inTmp);
          DeclareLocalVar(inTmp, null, null, s.Receiver, wr);
        }
        ...
        bool useOutParams = outTmps.Count > 0 && !returnStyleOuts;
        if (outTmps.Count > 0 && !useOutParams) {
          Contract.Assert(SupportsMultipleReturns || outTmps.Count == 1);
          wr.Write("{0} = ", Util.Comma(outTmps));
        }
        var wCall = EmitMethodCallModifiers(s.Method, outLhsTypes, wr);
        if (receiverReplacement != null) {
          wCall.Write(IdProtect(receiverReplacement));
          wCall.Write(".{0}", IdName(s.Method));
        } else if (customReceiver) {
          wCall.Write(TypeName_Companion(s.Receiver.Type, wCall, s.Tok, s.Method));
          wCall.Write(".{0}", IdName(s.Method));
        } else if (!s.Method.IsStatic) {
          TrParenExpr(s.Receiver, wCall);
          wCall.Write(".{0}", IdName(s.Method));
        } else {
          string qual, compileName;
          if (s.Method.IsExtern(out qual, out compileName) && qual != null) {
            wCall.Write("{0}.{1}", qual, compileName);
          } else {
            wCall.Write(TypeName_Companion(s.Receiver.Type, wCall, s.Tok, s.Method));
            wCall.Write(".{0}", IdName(s.Method));
          }
        ...
        EmitActualTypeArgs(typeArgs, s.Tok, wCall);
        wCall.Write("(");
        var nRTDs = EmitRuntimeTypeDescriptorsActuals(typeArgs, s.Method.TypeArgs, s.Tok, false, wCall);
        string sep = nRTDs == 0 ? "" : ", ";
        if (customReceiver) {
          TrExpr(s.Receiver, wCall);
          sep = ", ";
        }
        for (int i = 0; i < s.Method.Ins.Count; i++) {
          Formal p = s.Method.Ins[i];
          if (!p.IsGhost) {
            ...
            var w = EmitCoercionIfNecessary(s.Args[i].Type, s.Method.Ins[i].Type, s.Tok, wCall);
            if (s.Method.IsExtern(out _, out _)) {
    ...
    public static bool IsGhostLHS(Expression lhs) {
      lhs = lhs.Resolved;
    ...
    /// </summary>
    protected void TrParenExpr(string prefix, Expression expr, TargetWriter wr) {
    ...
    /// Before calling TrParenExpr(expr), the caller must have spilled the let variables declared in "expr".
    /// </summary>
    protected void TrParenExpr(Expression expr, TargetWriter wr) {
      Contract.Requires(expr != null);
      Contract.Requires(wr != null);
      wr.Write("(");
      TrExpr(expr, wr);
    ...
    /// </summary>
    protected void TrExprList(List<Expression> exprs, TargetWriter wr, Type/*?*/ type = null) {
    ...
    /// <summary>
    /// Whether the given variable needs to be copied to a temporary variable in
    /// order to appear inside a lambda.  By default, this is only true of out
    /// parameters, but for example Java needs to copy all variables used inside
    /// a lambda.
    /// </summary>
    protected virtual bool NeedsTempCopyForLambdaOccurrence(IVariable v) {
      return v is Formal f && !f.InParam;
    }

    /// <summary>
    /// Before calling TrExpr(expr), the caller must have spilled the let variables declared in "expr".
    /// </summary>
    protected void TrExpr(Expression expr, TargetWriter wr) {
      Contract.Requires(expr != null);
      Contract.Requires(wr != null);
      ...
      if (expr is LiteralExpr) {
        ...
        EmitCollectionDisplay(e.Type.AsSetType, e.tok, e.Elements, wr);

      } else if (expr is MultiSetDisplayExpr) {
        var e = (MultiSetDisplayExpr)expr;
        EmitCollectionDisplay(e.Type.AsMultiSetType, e.tok, e.Elements, wr);

      } else if (expr is SeqDisplayExpr) {
        var e = (SeqDisplayExpr)expr;
        EmitCollectionDisplay(e.Type.AsSeqType, e.tok, e.Elements, wr);

      } else if (expr is MapDisplayExpr) {
        var e = (MapDisplayExpr)expr;
        EmitMapDisplay(e.Type.AsMapType, e.tok, e.Elements, wr);
        ...
        if (sf != null) {
          ...
          if (NeedsCustomReceiver(e.Member)) {
            TrParenExpr(e.Obj, wr);
          }

          wr.Write(postStr);
        } else {
          EmitMemberSelect(w => TrExpr(e.Obj, w), e.Member, expr.Type).EmitRead(wr);
        }
        ...
        if (e.Seq.Type.IsArrayType) {
          if (e.SelectOne) {
            ...
            var w = EmitArraySelect(new List<Expression>() { e.E0 }, e.Type, wr);
            TrParenExpr(e.Seq, w);
          } else {
            EmitSeqSelectRange(e.Seq, e.E0, e.E1, true, wr);
          }
        } else if (e.SelectOne) {
          Contract.Assert(e.E0 != null && e.E1 == null);
          EmitIndexCollectionSelect(e.Seq, e.E0, wr);
        } else {
          EmitSeqSelectRange(e.Seq, e.E0, e.E1, false, wr);
        }
      } else if (expr is SeqConstructionExpr) {
        var e = (SeqConstructionExpr)expr;
        EmitSeqConstructionExpr(e, wr);
      } else if (expr is MultiSetFormingExpr) {
        var e = (MultiSetFormingExpr)expr;
        EmitMultiSetFormingExpr(e, wr);
      } else if (expr is MultiSelectExpr) {
        MultiSelectExpr e = (MultiSelectExpr)expr;
        WriteCast(TypeName(e.Type.NormalizeExpand(), wr, e.tok), wr);
        var w = EmitArraySelect(e.Indices, e.Type, wr);
        TrParenExpr(e.Array, w);

      } else if (expr is SeqUpdateExpr) {
        SeqUpdateExpr e = (SeqUpdateExpr)expr;
        if (e.ResolvedUpdateExpr != null) {
          TrExpr(e.ResolvedUpdateExpr, wr);
        } else {
          EmitIndexCollectionUpdate(e.Seq, e.Index, e.Value, wr);
        }

      } else if (expr is FunctionCallExpr) {
        FunctionCallExpr e = (FunctionCallExpr)expr;
        if (e.Function is SpecialFunction) {
          CompileSpecialFunctionCallExpr(e, wr, TrExpr);
        } else {
        ...
        EmitApplyExpr(e.Function.Type, e.tok, e.Function, e.Args, wr);
        ...
        switch (e.Op) {
          case UnaryOpExpr.Opcode.Not:
            if (e.Type.IsBitVectorType) {
              ...
              EmitUnaryExpr(ResolvedUnaryOp.BitwiseNot, e.E, wrTruncOperand);
            } else {
              EmitUnaryExpr(ResolvedUnaryOp.BoolNot, e.E, wr);
            }
            break;
          case UnaryOpExpr.Opcode.Cardinality:
            EmitUnaryExpr(ResolvedUnaryOp.Cardinality, e.E, wr);
            break;
          default:
        ...
        if (e.Exact) {
          ...
          Expression newExpr;
          var w = CopyFreeVarsToTemporaries(expr, NeedsTempCopyForLambdaOccurrence, out newExpr, wr);
          e = (LetExpr)newExpr;
          for (int i = 0; i < e.LHSs.Count; i++) {
            var lhs = e.LHSs[i];
            if (Contract.Exists(lhs.Vars, bv => !bv.IsGhost)) {
              var rhsName = string.Format("_pat_let{0}_{1}", GetUniqueAstNumber(e), i);
              w = CreateIIFE_ExprBody(e.RHSs[i], e.RHSs[i].Type, e.RHSs[i].tok, e.Body.Type, e.Body.tok, rhsName, w);
          ...
          if (missingBounds.Count != 0) {
            ...
            TrAssignSuchThat(new List<IVariable>(e.BoundVars).ConvertAll(bv => (IVariable)bv), e.RHSs[0], e.Constraint_Bounds, e.tok.line, w);
            EmitReturnExpr(e.Body, w);
          }
        }
        ...
        Expression newExpr;
        var wBody = CopyFreeVarsToTemporaries(expr, NeedsTempCopyForLambdaOccurrence, out newExpr, wr);
        e = (QuantifierExpr)newExpr;
        for (int i = 0; i < n; i++) {
          ...
          CompileCollection(bound, bv, false, wBody, e.Bounds, e.BoundVars, i);
        ...
        TrExpr(e.LogicalBody(true), wBody);
        ...
        Expression newExpr;
        wr = CopyFreeVarsToTemporaries(expr, NeedsTempCopyForLambdaOccurrence, out newExpr, wr);
        e = (SetComprehension)newExpr;
        ...
        TrExpr(e.Range, guardWriter);
        EmitCollectionBuilder_Add(e.Type.AsSetType, collection_name, e.Term, thn);
        ...
        TrExpr(e.Range, guardWriter);
        var termLeftWriter = EmitMapBuilder_Add(e.Type.AsMapType, e.tok, collection_name, e.Term, thn);
        if (e.TermLeft == null) {
          termLeftWriter.Write(IdName(bv));
        } else {
          TrExpr(e.TermLeft, termLeftWriter);
        }
        ...
        Expression newExpr;
        // Copy all free variables because this is a potentially long-lived closure,
        // not an IIFE or something passed to Quantifier and forgotten.
        wr = CopyFreeVarsToTemporaries(expr, which: (v => true), out newExpr, wr);
        var e = (LambdaExpr)newExpr;
        ...
        var e = (ITEExpr)expr;
        EmitITE(e.Test, e.Thn, e.Els, wr);

      } else if (expr is ConcreteSyntaxExpression) {
        var e = (ConcreteSyntaxExpression)expr;
        TrExpr(e.ResolvedExpression, wr);

      } else if (expr is NamedExpr) {
        TrExpr(((NamedExpr)expr).Body, wr);
      } else {
        Contract.Assert(false); throw new cce.UnreachableException();  // unexpected expression
      }
    }

    // Given an expression E of type T with free variables a, b, c, ... of types
    // A, B, C, ..., writes:
    //    ((Func<A, Func<B, Func<C, ... T>>>)
    //     (ta => tb => tc => ... => /* hole */))(a, b, c, ...))
    // and returns a TargetWriter positioned at the hole, along with
    // the new expression E[ta/a, tb/b, tc/c, ...], that is, the expression
    // with the temporaries substituted for the original free variables.
    //
    // Copies only the free variables selected by the given predicate, which
    // may be (v => true) to copy all free variables.  A common value for this
    // is the NeedsTempCopyForLambdaOccurrence method.
    TargetWriter CopyFreeVarsToTemporaries(Expression expr, Func<IVariable, bool> which, out Expression newExpr, TargetWriter wr) {
      List<BoundVar> bvars;
      List<Expression> fexprs;
      Translator.Substituter su;
      CreateFreeVarSubstitution(expr, which, out bvars, out fexprs, out su);
      ...
      if (bvars.Count == 0) {
        ...
        var w = EmitBetaRedex(bvars.ConvertAll(IdName), fexprs, typeArgs, bvars.ConvertAll(bv => bv.Type), expr.Type, expr.tok, wr);
        newExpr = su.Substitute(expr);
        return w;
      }
    }

    void CreateFreeVarSubstitution(Expression expr, Func<IVariable, bool> which, out List<BoundVar> bvars, out List<Expression> fexprs, out Translator.Substituter su) {
      Contract.Requires(expr != null);

      var fvs = new HashSet<IVariable>(Translator.ComputeFreeVariables(expr).Where(which));
      var sm = new Dictionary<IVariable, Expression>();
    ...
    void CompileSpecialFunctionCallExpr(FunctionCallExpr e, TargetWriter wr, FCE_Arg_Translator tr) {
      string name = e.Function.Name;

      if (name == "RotateLeft") {
        EmitRotate(e.Receiver, e.Args[0], true, wr, tr);
      } else if (name == "RotateRight") {
        EmitRotate(e.Receiver, e.Args[0], false, wr, tr);
      } else {
        CompileFunctionCallExpr(e, wr, tr);
      }
    }

    void CompileFunctionCallExpr(FunctionCallExpr e, TargetWriter wr, FCE_Arg_Translator tr) {
