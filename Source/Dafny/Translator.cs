...
#region first procedure, no impl yet
    ...
    void CheckWellformedWithResult(Expression expr, WFOptions options, Bpl.Expr result, Type resultType,
      ...
      if (expr is StaticReceiverExpr) {
        ...
        for (int i = 0; i < dtv.Ctor.Formals.Count; i++) {
          ...
          foreach (var p in Microsoft.Boogie.LinqExtender.Zip(dtv.Ctor.EnclosingDatatype.TypeArgs, dtv.InferredTypeArgs)) {
            su[p.Item1] = p.Item2;
          }
    ...
    private bool CanSafelyInline(FunctionCallExpr fexp, Function f) {
      ...
      visitor.Visit(f);
      return Microsoft.Boogie.LinqExtender.Zip(f.Formals, fexp.Args).All(formal_concrete => CanSafelySubstitute(visitor.TriggerVariables, formal_concrete.Item1, formal_concrete.Item2));
    }
